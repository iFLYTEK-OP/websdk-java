 ...
 */
public abstract class SimultInterpretationWebSocketListener extends WebSocketListener {

    private static final Logger logger = LoggerFactory.getLogger(SimultInterpretationWebSocketListener.class);

    /**
     * construction method
     */
    public SimultInterpretationWebSocketListener() {
    }
     ...
     */
    public abstract void onSuccess(WebSocket webSocket, SimultInterpretationResponse iatResponse);
    ...
    public void onMessage(WebSocket webSocket, String text) {
        ...
        try {
            if (text != null) {
                SimultInterpretationResponse sparkIatResponse = StringUtils.gson.fromJson(text, SimultInterpretationResponse.class);

                if (sparkIatResponse != null) {
                    onSuccess(webSocket, sparkIatResponse);
                } else {
                    onSuccess(webSocket, new SimultInterpretationResponse(-1, "simultInterpretion response error"));
                }
            }
        } catch (Exception e) {
