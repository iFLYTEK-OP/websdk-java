 ...
 * @date : 2021年04月02日
 */
public class QueryProgressTaskTest {
    @Test
    public void callTest() throws SignatureException, IOException, HttpException {
        ...
        message.setOk(-1);
        Gson gson = new Gson();
        PowerMockito.when(connector.post(Mockito.eq("https://raasr.xfyun.cn/api/getProgress"), Mockito.anyMap())).thenReturn(gson.toJson(message));
    ...
    public void callTest_IOException() throws SignatureException, IOException, HttpException {
        ...
        LfasrMessage message = new LfasrMessage();
        message.setOk(1);
        PowerMockito.when(connector.post(Mockito.eq("https://raasr.xfyun.cn/api/getProgress"), Mockito.anyMap())).thenThrow(IOException.class);
